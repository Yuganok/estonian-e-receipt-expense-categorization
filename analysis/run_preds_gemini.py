...
def main():
    parser = argparse.ArgumentParser(description="Generate Gemini predictions CSV")
    ...
    out = (
        Path("../output/evaluation/batch/preds_gemini.csv")
        if args.batch
        else Path("../output/evaluation/preds_gemini.csv")
    )
